...
class Server(threading.Thread):
    def __init__(self, db, host='0.0.0.0', port=7860):
        ...
        self.port = port
        self.mounts = {}
        self.server_url = f"http://{socket.gethostname()}:{port}"  #f"http://{socket.gethostbyname(socket.gethostname())}:{port}"
    ...
    def on_query(self, query):
        ...
        if isinstance(query, str):
            ...
            logging.debug(f"web image query  {query.size}  {type(query)}")
            query_type='image'
        else:
