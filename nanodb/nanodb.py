...
class NanoDB:
    def __init__(self, path=None, model="openai/clip-vit-large-patch14-336", dtype=np.float16, autosave=False, **kwargs):
        """
        kwargs:
    ...
    def scan(self, path, max_items=None, **kwargs):
        ...
        logging.info(f"added {len(indexes)} items to the index in from {path} ({time_elapsed:.1f} sec, {len(indexes)/time_elapsed:.1f} items/sec)")
    ...
    def load(self, path=None):
        ...
        logging.success(f"nanodb loaded {self.index.shape} records, {self.index.size()} bytes in {time.perf_counter()-time_begin:.2f} sec")
    ...
    def save(self, path=None):
        ...
        logging.success(f"nanodb wrote {self.index.shape} records, {self.index.size()} bytes in {time.perf_counter()-time_begin:.2f} sec")
    ...
    def embed(self, data, type=None, **kwargs):
        ...
        if type == 'image':
            embedding = self.model.vision.embed_image(data)
